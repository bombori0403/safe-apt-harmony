-- Default seat limit of 5 was far too low for real management companies
-- (a single complex can have 50+ staff). Raise the default and existing
-- orgs to effectively unlimited, and stop new-org signups from hard-coding 5.

ALTER TABLE public.organizations ALTER COLUMN seat_limit SET DEFAULT 1000;
UPDATE public.organizations SET seat_limit = 1000 WHERE seat_limit < 1000;

-- Recreate handle_new_user with the new org creating at the 1000 default
-- (only the seat_limit value changed from the previous version).
...
BEGIN
  ...
  ELSE
    ...
    VALUES (v_org_name, 'trial', 1000, now() + interval '14 days')
