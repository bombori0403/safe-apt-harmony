...
function Article({ no, title, children }: { no: string; title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-2">
      ...
      <div className="text-sm leading-7 text-foreground/90 whitespace-pre-wrap">{children}</div>
    </section>
  );
}
      ...
      <Card className="print:shadow-none print:border-0">
        ...
        <CardContent className="p-6 md:p-10 space-y-8">
          <section className="bg-muted/30 rounded-md p-5 space-y-3">
            ...
            <ul className="text-sm leading-7 list-disc pl-5 space-y-1">
              <li>근로자의 참여와 협의를 바탕으로 안전보건경영 시스템을 구축·운영한다.</li>
              <li>잠재적 유해·위험요인을 선제적으로 발굴하고 지속적으로 개선한다.</li>
              <li>관계 수급업체를 비롯한 이해관계자의 안전협력체계를 유지하고 성숙한 안전문화를 조성한다.</li>
              <li>안전보건관련 법령 및 규정을 철저히 준수한다.</li>
            </ul>
            <div className="text-sm font-semibold pt-2">
              위험성평가 중심의 「자기규율 예방체계」 확립 (중대재해 Zero, 산업재해 Zero)
            </div>
          </section>

          <Article no="1" title="목적">
            이 실시규정은 공무원연금공단 ○○○○상록아파트(이하 "공단 임대주택"이라 한다) 전체의 유해·위험요인을 파악하고, 그 유해·위험요인별 위험성의 수준을 결정한 후 위험성을 감소시키기 위해 필요한 조치를 마련하여 실시함을 목적으로 한다. 이 규정에서 정하지 않는 사항에 대해서는 고용노동부의 「사업장 위험성평가에 관한 지침」 및 「새로운 위험성평가 안내서」를 적용한다.
          </Article>

          <Article no="2" title="적용">
            이 실시규정은 공단 임대주택에서 수행하는 모든 작업, 설비 및 공정, 업무에 대한 활동, 또한 외부환경에서 발생할 수 있는 유해·위험요인 위험성평가에 대한 범위, 절차, 책임과 권한에 대하여 적용한다.
          </Article>

          <Article no="3" title="조직의 구성">
            위험성평가 실시 담당 조직의 구성은 아래표와 같이 하되, 공단 임대주택의 특성을 고려하여 구성하며, 담당자는 위·수탁용역업체 내부지침에 따른다.
          ...
          <Article no="4" title="역할과 책임">
            위험성평가 실시 담당 조직 구성원별 역할과 책임은 다음과 같이 한다.
            <div className="mt-3 overflow-x-auto">
              <table className="w-full text-xs border">
...
· 위험성평가 검토 및 결과에 대한 기록, 보관</td>
  ...
  5. 사업장 내에서 발생이 확인된 아차사고`}
          ...
          <Article no="6" title="실시시기">
            {`공단 임대주택 위험성평가 실시 시기는 다음과 같다.
  ...
  6. 필요 시 전담직원들에게 위험성평가 전문교육을 실시한다.`}
          ...
          <Article no="8" title="추진절차">
            {`위험성평가의 추진 절차는 다음과 같다.
  1. 1단계 : 사전준비
  ...
  5. 5단계 : 기록
    - 위험성평가를 수행한 결과를 관계자들에게 교육하거나 공유하기 위하여 기록한다.`}
          ...
          <Article no="9" title="위험성평가의 방법">
            공단 임대주택의 위험성평가 방법은 3단계 판단법을 사용한다.
          </Article>

          <Article no="10" title="위험성의 수준 판단 기준">
            {`위험성 수준과 그 판단 기준은 다음과 같다.`}
            <div className="mt-3 overflow-x-auto">
              <table className="w-full text-xs border">
                <thead className="bg-muted">
                  <tr>
...
· 휴업을 요하지 않는 경미한 부상 또는 질병이 예상되는 경우</td>
          ...
          <Article no="11" title="근로자에 대한 공유">
            {`  1. 근로자들이 많이 다니고 잘 볼 수 있는 곳에, 잘 볼 수 있는 방법(가독성 높은 큰 글씨, 전광판 등)으로 위험성평가 결과 게시
  2. 안전보건교육 내용에 교육 대상 근로자의 작업(공정)에 대한 위험성평가 결과 내용 포함
  3. 작업 전 안전점검회의 시 위험성평가 내용 포함`}
          </Article>

          <Article no="12" title="근로자의 참여 방법">
            공단 임대주택의 위험성평가 대상 작업(공정)의 모든 과정에 근로자 1명 이상 참여하도록 한다.
          </Article>

          <Article no="13" title="유의사항">
            {`① 산업안전보건법 기타 요구사항에 적합한 상태인지를 확인하고 미달하고 있는 경우에는 사업주에게 보고한 후 위험성 수준이 높은 것부터 우선적으로 위험성 감소대책을 반영하여 개선한다.
...
④ 위험성평가 참여자는 위험성 결정 시 최악의 상황에서 가장 큰 부상 또는 질병을 기준으로 판단한다.`}
          ...
          <Article no="14" title="점검 및 개선활동">
            {`① 위험성평가의 이행에 대한 점검은 위험성평가 담당자 및 이행 책임자가 확인하여야 한다.
② 위험성평가의 이행 점검 결과, 미이행 사항이나 추가적 유해·위험요인이 발견된 경우 시정조치를 하여야 하며, 시정조치 내용은 차기(다음번) 위험성평가에 반영되도록 하여야 한다.`}
          </Article>

          <Article no="15" title="기록">
            {`① 위험성평가 기록은 사업주에게 승인을 받는다.
...
③ 위험성평가 기록물은 연 1회 정도 정기적으로 검토하고, 수정·보완이 필요한 경우에는 근로자의 의견을 반영한 후에 변경 여부를 결정하며, 모든 근로자가 알 수 있도록 배부 또는 게시한다.`}
          ...
          <Article no="16" title="재평가">
            {`① 위험성평가 기록물은 연 1회 정기적으로 검토 및 재평가하고, 개선 방안이 적절치 않아 수정·보완이 필요한 경우에는 근로자의 의견을 수렴한 후에 변경 여부를 결정하며, 이를 당해연도 정기위험성평가 내용에 반영한다. 이 내용을 모든 근로자가 알 수 있도록 배부 또는 게시한다.
② 전년도 위험성평가 미이행 사항 또는 시행예정 사항을 포함하여 재평가를 실시하여야 한다.`}
          </Article>

          <Article no="17" title="추진절차 일정">
              ...
              <table className="w-full text-xs border">
                ...
                <tbody className="align-top">
                  {[
                    ...
                    ["7","재료/물질","에어로졸, 생물학적 시료, 가연성/인화성 물질, 먼지, 폭약, 흄, 가스, 미스트, 산화제","호흡곤란·질식, 암, 부식, 폭발, 화재, 감염, 중독, 과민성"],
