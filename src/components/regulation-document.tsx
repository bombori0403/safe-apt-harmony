...
function F({ k, className }: { k: string; singleLine?: boolean; className?: string }) {
  const get = useContext(GetCtx);
  return <span className={"whitespace-pre-wrap " + (className ?? "")}>{get(k)}</span>;
}

function TableBlock({ k }: { k: string }) {
  const get = useContext(GetCtx);
  return (
    <pre className="text-xs leading-6 whitespace-pre-wrap border rounded-md p-3 bg-muted/30 font-sans break-inside-avoid">
      {get(k)}
    </pre>
  );
}
  ...
  return (
    <div className="border border-slate-400 rounded-sm overflow-hidden bg-white text-center text-xs min-w-[110px]">
      <div className="bg-sky-100 border-b border-slate-400 px-2 py-1 font-semibold">
        <F k={roleKey} className="text-center" />
      </div>
      <div className="px-2 py-1">
        <F k={nameKey} className="text-center" />
      </div>
    </div>
  );
}
...
function OrgChart() {
  ...
  return (
    <div className="border rounded-md p-4 md:p-6 bg-white overflow-x-auto break-inside-avoid">
